...
def convert_separate(year):
    ...
    res.index += 1
    res.to_excel("D:/КПІ/3-курс/Data Science/project/DS_IndexProsperity/SensitivityAnalysis_Result" + f"{year}result.xlsx")
    ...
    # safety index cleaning
    safety = pd.read_excel("D:/КПІ/3-курс/Data Science/project/DS_IndexProsperity/SensitivityAnalysis_Data/Safety.xlsx")
    names = ['Control of Corruption: Estimate', 'Government Effectiveness: Estimate',
             'Political Stability and Absence of Violence/Terrorism: Estimate', 'Regulatory Quality: Estimate',
    ...
    # deleting unnecessary information
    to_delete = ['Rail lines (total route-km)', 'Net migration',
    ...
    data['Economic'] = data[economic].mean(axis=1)
    data['Environment'] = data[environment].mean(axis=1)
    data['Education'] = (data[education[0]] * 0.5 + (
                data[education[1]] * 0.5 / 3 + data[education[2]] * 0.5 / 3 + data[education[3]] * 0.5 / 3))
    data['Health'] = (data[health[0]] * 0.3 + data[health[1]] * 0.7)
    data['Safety'] = 0
    for i in loadings.columns:
        coef = expl_var[i] / expl_var.sum()
    ...
    data['Prosperity index'] = data['Environment'] * coef_for_environment + coef_for_others * (
                data['Economic'] + data['Education'] + data['Health'] + data['Safety'])

    # loading to excel normalized version with indexes
    data.to_excel("D:/КПІ/3-курс/Data Science/project/SensitivityAnalysis_Data" + f"test{year}.xlsx")
    ...
    res.reset_index(names="Rank", inplace=True)
    res.index += 1
    res.to_excel("D:/КПІ/3-курс/Data Science/project/DS_IndexProsperity/SensitivityAnalysis_Result" + f"test{year}result.xlsx")